...
## 1-2. 연결 요청 제출 후 — **기다리는 것 말고 할 수 있는 게 없다**

2026-09-10 제출 완료. 앱 **Pickdam Markto · 앱 ID 1610207**, 상태 **"Trial 액세스 대기 중"**.

앱 상세(`developers.pinterest.com/apps/1610207/`)에서 실측한 것:

- **앱 비밀 키**: "trial 액세스 대기 중 중에는 사용할 수 없음" — 값이 나오지 않는다.
- **리디렉션 URI 입력칸이 `disabled`다.** 화면에는 보이지만 입력이 막혀 있다.
- 즉 **승인 전에는 `oauth_setup.py`를 돌릴 수 없다.** 앱 ID만으로는 토큰 교환이 안 된다.

페이지에 "액세스 토큰 생성하기" 버튼이 있지만 **이 경로는 쓸 수 없다** —
주는 스코프가 `pins:read, boards:read, user_accounts:read, ads:read, catalogs:read` 뿐이라
**`pins:write`가 없다.** 핀을 만들 수 없으므로 영상 촬영에도, 실제 게시에도 못 쓴다.

승인이 나면 그때 리디렉션 URI에 `http://localhost:8085/`를 넣는다 —
화면 안내가 "리디렉션 URI는 https, 앱별 체계 또는 **localhost가 있는 http**를 사용해야 합니다"라고
명시하므로 이 값이 맞다. 그다음 앱 비밀 키를 받아 3번(영상)으로 간다.
